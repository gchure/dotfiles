/* This is a configuration file for the ultra-lightweight OSX window manager 
 * Phoenix (https://github.com/kasper/Phoenix). This script is heavily
 * modified from teetrinker's vim-like config file 
...
var active = false;
api.bind('space', ['alt'], function() { if (!active) {
    enableKeys();
  } else {
    disableKeys();
  }
...
bind( 'j', mCmd, function() {
  Window.focusedWindow().toGrid(0, 0.7, 1, 0.3);
});

bind( 'k', mCmd, function() {
  Window.focusedWindow().toGrid(0, 0, 1, 0.3);
});

//Set up the windows in quadrants.


//Upper-left
bind('h', mCtrl, cycleCalls(
	toGrid,
	[
	[0, 0, 0.5, 0.5],
	[0.2, 0.1, 0.3, 0.4]
...
// The cursor keys move the focused window.
bind( 'k', mNone, function() {
...
// <SHIFT> + cursor keys grows/shrinks the focused window.
bind( 'h', mShift, function() {
  Window.focusedWindow().growWidth();
});

bind( 'l', mShift, function() {
  Window.focusedWindow().shrinkWidth();
});

bind( 'k', mShift, function() {
  Window.focusedWindow().shrinkHeight();
});

bind( 'j', mShift, function() {
...
bind( '1', mNone, function() {
  var forklift = App.findByTitle('iTerm2').firstWindow();
  if (forklift) {
